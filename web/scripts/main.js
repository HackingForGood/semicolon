...
function FriendlyChat() {
  ...
  this.messageCardContainer = document.getElementById('messages-card-container');
...
// Loads chat messages history and listens for upcoming ones.
FriendlyChat.prototype.loadCourses = function() {
  // Reference to the /messages/ database path.
  ...
  this.coursesRef.limitToLast(12).on('child_added', setCourse);
  this.coursesRef.limitToLast(12).on('child_changed', setCourse);
};
...
// Displays a Course in the UI.
FriendlyChat.prototype.displayCourse = function(key, name, text, picUrl, imageUri) {
  var div = document.getElementById(key);
  // If an element for that message does not exists yet we create it.
  if (!div) {
    var container = document.createElement('div');
  ...
  div.querySelector('.name').textContent = name;
  var messageElement = div.querySelector('.course');
  if (text) { // If the message is text.
    messageElement.textContent = text;
    // Replace all line breaks by <br>.
    messageElement.innerHTML = messageElement.innerHTML.replace(/\n/g, '<br>');
